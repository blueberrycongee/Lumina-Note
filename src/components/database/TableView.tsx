...
export function TableView({ dbId }: TableViewProps) {
  ...
  const handleCellSaveState = useCallback(
    (
      rowId: string,
      columnId: string,
  ...
  const focusCell = useCallback(
    (rowIndex: number, columnIndex: number, startEditing = false) => {
  ...
  const handleCellBlur = useCallback(
    (rowIndex: number, colIndex: number, action?: CellCommitAction) => {
      setEditingCell(null);
  ...
  return (
    <div
      className="h-full overflow-x-auto overflow-y-auto bg-background"
      ref={tableRef}
    >
      <table
            ...
            >
                ...
                <div className="flex items-center justify-center h-9">
                  {hoveredRow === row.id ? (
                    <div className="relative">
                      <DatabaseIconButton
                        onClick={() =>
                          setRowMenuOpen(rowMenuOpen === row.id ? null : row.id)
                        }
              ...
              {columns.map((column, colIndex) => (
                <td
                    ...
                    editingCell?.columnId === column.id
                      ...
                      : cellStatus[getCellKey(row.id, column.id)] === "saving"
                        ...
                        : cellStatus[getCellKey(row.id, column.id)] === "saved"
                          ? "bg-primary/[0.1]"
                          : cellStatus[getCellKey(row.id, column.id)] ===
                              "error"
                  ...
                  onKeyDown={(e) =>
                    handleCellKeyDown(e, rowIndex, colIndex, row.id, column.id)
                  }
                  tabIndex={
                    editingCell?.rowId === row.id &&
                    editingCell?.columnId === column.id
                ...
                >
                  ...
                  {cellStatus[getCellKey(row.id, column.id)] === "error" && (
                    <span
                      className="pointer-events-none absolute right-1 top-1 text-destructive"
                      aria-hidden
                    >
                  ...
                  <div className="flex items-center">
                    <div className="min-w-0 flex-1">
                      <DatabaseCell
                        ...
                        onBlur={(action) =>
                          handleCellBlur(rowIndex, colIndex, action)
                        }
                        onSaveStateChange={(state) =>
                          handleCellSaveState(row.id, column.id, state)
                        }
                      />
                    </div>
                      ...
                      <div className="mr-1 h-8 w-8 flex-shrink-0">
                        <DatabaseIconButton
                          ...
                          className={`h-8 w-8 transition-opacity duration-120 ease-out ${
                            hoveredRow === row.id && row.notePath
                              ? "opacity-100"
                              : "pointer-events-none opacity-0"
                          }`}
                          tabIndex={
                            hoveredRow === row.id && row.notePath ? 0 : -1
                          }
                          aria-hidden={
                            hoveredRow === row.id && row.notePath
                              ? undefined
