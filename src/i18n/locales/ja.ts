...
export default {
  ...
  sidebar: {
    ...
    openClawCronPluginUnavailable:
      "OpenClaw プラグインコマンドが利用できません。プラグインが有効か確認してください。",
    ...
    openClawCronPayloadMessagePlaceholder:
      "メッセージまたはイベントテキストを入力",
  ...
  overview: {
    ...
    openClawDesc:
      "Lumina は OpenClaw と同じファイル群を操作しています。認識済み artifact ファイル数: {count}。",
  ...
  profilePreview: {
    ...
    recentEmpty:
      "公開ノートがありません。frontmatter に visibility: public を設定してください。",
  ...
  plugins: {
    ...
    title: "プラグイン",
    intro:
      "Lumina はインストール場所付近のデフォルトプラグインフォルダを優先し、ワークスペース .lumina/plugins、ユーザープラグインディレクトリ、内蔵リソースも読み込みます。",
    ...
    openWorkspaceFolderHint:
      "ファイルマネージャーでデフォルトプラグインフォルダを開きます",
  ...
  settingsModal: {
    ...
    openClawTitle: "OpenClaw Workspace",
    openClawDesc:
      "外部の OpenClaw ワークスペースを現在の Lumina ワークスペースに接続し、共有される実ファイル境界を明示します。",
    openClawEnabled: "統合を有効化",
    openClawDisabledDesc:
      "OpenClaw integration は現在無効です。再度有効化するまで Lumina は通常のフォルダとして扱います。",
    openClawReleaseFlagDesc:
      "現在の release flag では OpenClaw integration が無効です。この build で有効化するには VITE_ENABLE_OPENCLAW_WORKSPACE=1 を設定してください。",
    openClawNoWorkspace:
      "まずローカルワークスペースを開いてから、OpenClaw ワークスペースかどうかを確認してください。",
    ...
    openClawMountedWorkspaceHint:
      "外部の OpenClaw ワークスペースパスを現在の Lumina ワークスペースに接続します。",
    openClawPathRequiredTitle: "OpenClaw ワークスペースが未選択です",
    openClawPathRequiredDesc:
      "再検出または接続の前に、外部 OpenClaw ワークスペースのパスを選択してください。",
    ...
    openClawBoundaryHint:
      "Lumina での編集は OpenClaw が使用する同じファイルに直接書き戻されます。これはインポートされたコピーではなく、接続されたワークスペースです。",
  ...
  videoNote: {
    ...
    pasteVideoUrl:
      "B站動画リンクを貼り付け、例: https://www.bilibili.com/video/BV...",
  ...
  pdfViewer: {
    ...
    mockLongText:
      "これは要素認識や操作のテスト用に用意した長めのサンプルテキストです。",
