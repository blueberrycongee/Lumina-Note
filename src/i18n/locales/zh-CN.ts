...
export default {
  ...
  overview: {
    ...
    openClawDesc:
      "Lumina 正在操作 OpenClaw 的同一批文件。当前识别到的产物文件数：{count}。",
  ...
  plugins: {
    ...
    title: "插件",
    intro:
      "Lumina 会优先从默认插件目录（安装位置旁）加载插件，并兼容工作区 .lumina/plugins、用户插件目录和内置资源。",
  ...
  settingsModal: {
    ...
    openClawTitle: "OpenClaw 工作区",
    openClawDesc:
      "把一个外部 OpenClaw 工作区挂到当前 Lumina 工作区，并明确这是一套共享真实文件而不是导入副本。",
    openClawEnabled: "启用集成",
    openClawDisabledDesc:
      "OpenClaw 集成当前已关闭。重新启用前，Lumina 会把该工作区当作普通文件夹处理。",
    openClawReleaseFlagDesc:
      "当前发布开关已关闭 OpenClaw 集成。为这个构建设置 VITE_ENABLE_OPENCLAW_WORKSPACE=1 后才能启用。",
    openClawNoWorkspace:
      "请先打开一个本地工作区，再检查它是否为 OpenClaw 工作区。",
    ...
    openClawMountedWorkspaceHint:
      "把一个外部 OpenClaw 工作区路径挂到当前 Lumina 工作区上。",
    openClawPathRequiredTitle: "尚未选择 OpenClaw 工作区",
    openClawPathRequiredDesc:
      "先选择一个外部 OpenClaw 工作区路径，再进行检测或附着。",
    ...
    openClawBoundaryHint:
      "你在 Lumina 里的修改会直接写回 OpenClaw 正在使用的同一批文件。这是附着工作区，不是导入副本。",
...
5) 输出简洁、分条，使用中文`,
  ...
  imageManager: {
    ...
    batchActions: "批量操作",
    batchSummary:
      "{orphanCount} 个孤立，{multiCount} 个多引用，总大小 {totalSize}。",
