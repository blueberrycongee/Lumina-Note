...
describe("thinking-options (opencode bridge translator)", () => {
  ...
  describe("DeepSeek V4", () => {
    it("emits both extra_body (Python SDK) and top-level thinking (Vercel AI SDK)", () => {
      // @ai-sdk/openai-compatible forwards unknown providerOptions keys
      // directly into the request body, so `thinking` at the top level is
      // what DeepSeek HTTP API actually reads. `extra_body` is kept for
      // backward compat with the Python OpenAI SDK path.
      expect(
        buildModelOptionsBlob({
          provider: "deepseek",
